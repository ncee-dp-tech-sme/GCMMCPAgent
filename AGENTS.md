...
**Root Cause:**
Debug logs were being generated by the agent but not appearing in the Debug UI because the `debug_ui` instance wasn't being passed through the agent creation chain.

**The Problem:**
1. `chat_ui.py` created a `debug_ui` instance (line 217)
2. But `create_gcm_agent()` didn't accept a `debug_ui` parameter
3. The `GCMAgent` constructor accepts `debug_ui` but it wasn't being passed through
4. Result: `ObservabilityLogger` was created with `debug_ui=None`, so logs went to console only

**The Fix:**
1. **gcm_agent/agent/__init__.py** (lines 52, 141):
   - Added `debug_ui: Optional['DebugUI'] = None` parameter to `create_gcm_agent()`
   - Pass `debug_ui` to `GCMAgent` constructor
   - Updated docstring to document the parameter

2. **gcm_agent/ui/chat_ui.py** (line 221):
   - Pass `debug_ui=debug_ui` when calling `create_gcm_agent()`

**How It Works:**
- Debug UI instance created before agent initialization
- Passed through `create_gcm_agent()` → `GCMAgent.__init__()` → `ObservabilityLogger`
- `ObservabilityLogger._send_to_debug_ui()` now has valid debug_ui reference
- All observability logs (tool selection, token usage, performance) now appear in Debug UI

**Impact:**
- ✅ Debug UI now displays real-time observability logs
- ✅ Tool selection reasoning visible in UI
- ✅ Token usage tracking visible in UI
- ✅ Performance metrics visible in UI
- ✅ No breaking changes - debug_ui parameter is optional

**Files Modified:**
- [`gcm_agent/agent/__init__.py`](gcm_agent/agent/__init__.py:52,141) - Added debug_ui parameter
- [`gcm_agent/ui/chat_ui.py`](gcm_agent/ui/chat_ui.py:221) - Pass debug_ui to agent creation


### Fixed Query Independence - Context Bleeding Prevention (2026-06-09 22:45 UTC)

**Root Cause:**
The agent was incorrectly carrying over filters from previous queries (especially failed queries) to new queries. The LLM treated filters from conversation history as persistent state, even when the new query explicitly requested "all" items.
...
**The Fix ([`gcm_agent/agent/prompts.py`](gcm_agent/agent/prompts.py:11-30)):**
Enhanced system prompt with explicit query independence instructions:

1. **Added Core Instruction #6:**
   - "CRITICAL: Each query is independent - do NOT carry over filters from previous queries unless explicitly requested"
...
**Note on 500 Error:**
The original 500 error when filtering by hostname is a **server-side issue** with the GCM MCP server's API endpoint and requires server-side investigation. This fix addresses the context bleeding behavior, not the underlying server error.

**Files Modified:**
- [`gcm_agent/agent/prompts.py`](gcm_agent/agent/prompts.py:11-30) - Added query independence instructions
- [`docs/QUERY_INDEPENDENCE_FIX.md`](docs/QUERY_INDEPENDENCE_FIX.md) - Full documentation

## Recent Updates (2026-06-09)

### Fixed Table Formatter Multi-Table Detection (2026-06-09 22:40 UTC)

**Root Cause:**
When querying details after listing all items, both the broad list table AND the filtered detail table were displayed. The detection logic didn't handle multiple tables in a single response.
...
Result: Both tables displayed, creating visual clutter

**The Fix ([`gcm_agent/utils/table_formatter.py`](gcm_agent/utils/table_formatter.py:145-157)):**
Added multi-table detection:
1. After finding first table, check if second table exists in after-table content
2. If second table found, hide first (broader) table
3. Show only content after first table (includes second more-specific table)
4. Recursively format any tables in the after-table content
...
**Files Modified:**
- [`gcm_agent/utils/table_formatter.py`](gcm_agent/utils/table_formatter.py:145-157) - Added multi-table detection
- [`tests/test_multi_table_detection.py`](tests/test_multi_table_detection.py) - Comprehensive test suite (3 tests)
- [`docs/TABLE_FORMATTER_MULTI_TABLE_FIX.md`](docs/TABLE_FORMATTER_MULTI_TABLE_FIX.md) - Full documentation

### Fixed Table Formatter Smart Detection (2026-06-09 22:32 UTC)

**Root Cause:**
The smart detection logic in table formatter was too aggressive, hiding tables when listing assets with explanatory text.

**The Fix ([`gcm_agent/utils/table_formatter.py`](gcm_agent/utils/table_formatter.py:158-182)):**
Made detection more strict:
1. Requires 10+ meaningful lines (increased from 5)
2. Requires 60%+ of lines to be structured field:value pairs
3. Excludes lines starting with "If " or "The " from field:value detection

**Files Modified:**
- [`gcm_agent/utils/table_formatter.py`](gcm_agent/utils/table_formatter.py:158-182) - Updated smart detection logic
- [`tests/test_table_formatter_fix.py`](tests/test_table_formatter_fix.py) - Comprehensive test suite (4 tests)
- [`docs/TABLE_FORMATTER_FIX.md`](docs/TABLE_FORMATTER_FIX.md) - Full documentation

### Chat UI Refactoring for Maintainability (2026-06-09 20:45 UTC)
### Smart Table Detection for Detail Queries (2026-06-09 22:00 UTC)

**Fixed duplicate table display when querying asset/key details after listing all items.**

**Problem:**
When asking "show all assets" followed by "show details for asset X", the agent would include the full asset list table as context, then show the asset details. Both tables were displayed, creating visual clutter.

**Solution:**
Enhanced `format_response_tables()` in [`gcm_agent/utils/table_formatter.py`](gcm_agent/utils/table_formatter.py) with intelligent content detection:

1. **Detects detail queries**: When response has 5+ meaningful lines after a table with field:value pairs
2. **Hides context tables**: Suppresses the list table when detail content is present
3. **Recursive formatting**: Still formats tables within the detail content (e.g., detail field tables)

**Detection Logic:**
- Counts meaningful lines after first table
- Checks for field:value patterns (`:` or `**bold labels**`)
- If both conditions met → hide first table, show only details
- Recursively format any tables in detail content

**Impact:**
- Detail queries now show only relevant information
- List queries still show formatted tables normally
- Eliminates duplicate/context table clutter
- Improves readability for follow-up detail queries

**Files Modified:**
- [`gcm_agent/utils/table_formatter.py`](gcm_agent/utils/table_formatter.py:118-189) - Added smart detection logic
- [`tests/test_table_formatter_smart_detection.py`](tests/test_table_formatter_smart_detection.py) - Comprehensive test suite (5 tests)
- [`docs/TABLE_FORMATTING.md`](docs/TABLE_FORMATTING.md) - Updated documentation

**Test Coverage:**
✅ Detail queries hide context tables  
✅ List queries show tables normally  
✅ Short summaries don't trigger hiding  
✅ Non-table content unchanged  
✅ Content without field:value pairs shows tables
