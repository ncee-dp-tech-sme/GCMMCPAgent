...
**Root Cause:**
The smart detection logic in table formatter was too aggressive, hiding tables when listing assets with explanatory text. When querying "list all details of the asset with hostname kushaq.dev.fyre.ibm.com", the table with 21 assets was hidden, showing only the first asset's details.

**The Problem:**
Old logic triggered on:
1. 5+ meaningful lines after table
2. Any line containing `:` (field:value pairs)
3. Result: Explanatory text like "The list continues..." with a few field:value pairs incorrectly triggered detail mode
